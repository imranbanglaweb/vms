        ...
        <div class="card-body p-4 bg-light">
            <form id="vehicleForm" action="{{ isset($vehicle) ? route('vehicles.update', $vehicle->id) : route('vehicles.store') }}" method="POST">
                ...
                <div class="row gy-3 gx-4 align-items-center">
                    ...
                    {{-- RTA Office Dropdown --}}
                    <!-- <div class="col-md-4">
                        <label class="form-label fw-semibold small mb-1">RTA Office *</label>
                        <div class="input-group input-group-sm shadow-sm rounded">
                            <span class="input-group-text bg-white border-end-0">
                                <i class="fa fa-landmark text-secondary"></i>
                            </span>
                            <select name="rta_office_id" class="form-select border-start-0 py-2">
                                <option value="">Select Office</option>
                                @foreach($rtaOffices as $id => $name)
                                    <option value="{{ $id }}" {{ old('rta_office_id', $vehicle->rta_office_id ?? '') == $id ? 'selected' : '' }}>
                                        {{ $name }}
                                    </option>
                                @endforeach
                            </select>
