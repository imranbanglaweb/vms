    ...
    <form id="assignForm" action="{{ route('transport.approvals.assign', $requisition->id) }}" method="POST" class="mt-4" onsubmit="event.preventDefault(); submitAssign();">
        ...
        <div class="row">
            <div class="col-md-5">
                ...
                <select name="assigned_vehicle_id" class="form-select form-select-lg select2" style="width:100%">
            ...
            <div class="col-md-5">
                ...
                <select name="assigned_driver_id" class="form-select form-select-lg select2" style="width:100%">
                    <option value="">-- Select Driver --</option>
                    @foreach($drivers as $driver)
                        <option value="{{ $driver->id }}" data-status="{{ $driver->availability_status }}"
                                        @if(strtolower($driver->availability_status) === 'assigned') disabled @endif {{ (isset($requisition->assignedDriver) && $requisition->assignedDriver->id == $driver->id) ? 'selected' : '' }}>
                            {{ $driver->driver_name }}@if(!empty($driver->phone)) - [{{ $driver->phone }}]@endif
                        </option>
                    @endforeach
                </select>
